...
Uso:
  ...
  python tools/plot_task.py logs/task_20240101_120000.csv --target 0.5

Saída:
  - PNG com o mesmo nome do CSV no mesmo diretório
  - Métricas impressas no terminal e em uma caixa no gráfico
...
def median(values):
    """Mediana simples, sem dependências externas."""
    if not values:
        return None
    s = sorted(values)
    n = len(s)
...
def estimate_object_position(lidar_dist, lidar_valid, yolo_angle, yolo_found,
    ...
    Retorna a mediana (robusta a ruído) de todas as estimativas, ou None.

    Convenção: ângulo do YOLO positivo = objeto à direita (sentido horário),
    logo o rumo do objeto no mundo é (yaw - ângulo).
    """
    xs, ys = [], []
...
def compute_metrics(t, lidar_dist, lidar_valid, lidar_min,
    ...
    """Calcula as métricas de validação do experimento."""
    m = {}

    # Distância ao alvo: inicial, final e erro
    valid_idx = [i for i, v in enumerate(lidar_valid) if v]
    if valid_idx:
        d0 = lidar_dist[valid_idx[0]]
        df = lidar_dist[valid_idx[-1]]
        m['dist_inicial'] = d0
        m['dist_final']   = df
        m['erro_final']   = df - target_dist
    else:
        m['dist_inicial'] = m['dist_final'] = m['erro_final'] = None

    # Tempo de acomodação: primeiro instante em que |d - alvo| <= tol e assim
    # permanece (dentro de 2*tol) até o fim. Só vale se terminou perto do alvo.
    m['t_acomodacao'] = None
    ...
    # Obstáculo mais próximo e acionamento do freio
    ...
    # Ângulo do alvo: inicial e final (só deteções válidas)
    ...
    # Comprimento do caminho percorrido (odometria)
    path = 0.0
...
def format_metrics(m, target_dist):
    """Monta as linhas de texto das métricas para terminal e figura."""
...
def plot(path, target_dist):
    ...
    # Métricas e posição estimada do objeto
    ...
    # caixa com erro final e acomodação
    ...
    # 6. Trajetória (odometria) + objeto estimado
    ax = axes[2, 1]
    ...
    if obj_pos is not None:
        ...
        ax.add_patch(circ)
        # garante que o objeto e o círculo caibam no eixo
    ...
    # Caixa de métricas no rodapé da figura
...
if __name__ == '__main__':
    ...
    plot(args.csv_file, args.target)